...
![作成一覧画面](images/reservation-create.png)
...
- GitHub Copilot（コメント駆動での提案活用。自動補完は無効化し、自分でロジックを考えてから提案を参考にするスタイルで活用）

## テスト

Pestで正常系・異常系（権限のないユーザーによる操作の拒否）を中心にテストを作成しています。
...
- **大量データを想定したパフォーマンス改善**：現状はN+1クエリ対策や`select`での列指定を一部行っていますが、店舗数・予約数が大幅に増えた場合に備え、インデックス設計の見直しやキャッシュの活用、オーナー側の予約集計処理（現在はコレクション操作で実装）のクエリベースへの置き換えなどを今後検討しています。
