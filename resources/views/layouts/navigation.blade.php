                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @auth
                        ...
                        @if(auth()->user()->isOwner())
                            <x-nav-link :href="route('owner.reservations.index')" :active="request()->routeIs('owner.reservations.*')">
                                {{ __('予約管理') }}
                            </x-nav-link>
                        @endif
                    @endauth
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @auth
                ...
                @if(auth()->user()->isOwner())
                    <x-responsive-nav-link :href="route('owner.reservations.index')" :active="request()->routeIs('owner.reservations.*')">
                        {{ __('予約管理') }}
                    </x-responsive-nav-link>
                @endif
            @endauth
        ...
        @else
            <div class="pt-4 pb-1 border-t border-gray-200 px-4 space-y-2">
                <a href="{{ route('login') }}" class="block text-gray-600">ログイン</a>
                <a href="{{ route('register') }}" class="block text-blue-500">新規登録</a>
            </div>
        @endauth
    </div>
</nav>
